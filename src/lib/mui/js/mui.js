...
(function($, document) {
	$(function() {
		...
		document.addEventListener('focusin', function(e) {
			...
			var target = e.target;
			//TODO 需考虑所有键盘弹起的情况
			if (target.tagName && (target.tagName === 'TEXTAREA' || (target.tagName === 'INPUT' && (target.type === 'text' || target.type === 'Search.vue' || target.type === 'number')))) {
...
(function($, window, document) {
	...
	var Input = function(element, options) {
		...
		if (~this.options.actions.indexOf('slider')) { //slider
			...
			if (~this.options.actions.indexOf('Search.vue')) {
	...
	$.fn.input = function(options) {
		...
		this.each(function() {
			...
			if (this.type === 'range' && row.classList.contains('mui-input-range')) {
				...
				if (this.type === 'Search.vue' && row.classList.contains('mui-search')) {
					actions.push('Search.vue');
				}
			}
